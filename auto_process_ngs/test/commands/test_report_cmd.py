...
"""
    ...
    def test_report_projects_with_custom_metadata_explicit_values(self):
        """report: report run in 'projects' mode with custom metadata & explicit values
        """
        ...
        with open(local_settings_file, "wt") as fp:
            ...
            custom~_project_metadata: order_numbers,assigned_to
            """))
        # Make a mock auto-process directory
        mockdir = MockAnalysisDirFactory.bcl2fastq2(
            '170901_M00879_0087_000000000-AGEW9',
        ...
        # Generate projects report
        expected = """MISEQ_170901#87\tAB\tStephen Taylor\t#000124
MISEQ_170901#87\tCDE\t\t
"""
        for o,e in zip(report_projects(ap, fields=["run_id","project","assigned_to","order_numbers"]).split('\n'),
                       expected.split('\n')):
            self.assertEqual(o,e)

    def test_report_projects_with_custom_metadata_missing_values(self):
        """report: report run in 'projects' mode with custom metadata & missing values
        """
        # Make local settings file and define custom project metadata items
        local_settings_file = os.path.join(self.dirn, "auto_process_test.ini")
        with open(local_settings_file, "wt") as fp:
            fp.write(dedent("""[metadata]
            custom_project_metadata: order_numbers,assigned_to
            """))
        # Make a mock auto-process directory
        mockdir = MockAnalysisDirFactory.bcl2fastq2(
            '170901_M00879_0087_000000000-AGEW9',
...
"""
    ...
    def test_report_projects_with_custom_metadata_no_config_explicit_values(self):
        """report: report run in 'projects' mode with custom metadata, no config & explicit values
        """
        # Make a mock auto-process directory
        mockdir = MockAnalysisDirFactory.bcl2fastq2(
            '170901_M00879_0087_000000000-AGEW9',
...
"""
    ...
    def test_report_projects_with_custom_metadata_no_config_missing_values(self):
        """report: report run in 'projects' mode with custom metadata, no config & missing values
        """
        # Make a mock auto-process directory
        mockdir = MockAnalysisDirFactory.bcl2fastq2(
            '170901_M00879_0087_000000000-AGEW9',
        ...
        # Make autoprocess instance and set required metadata
        ap = AutoProcess(analysis_dir=mockdir.dirn)
        # Attempt to generate projects report should fail
        self.assertRaises(KeyError,
                          report_projects,
                          ap,
                          fields=["run_id","project","assigned_to","order_numbers"])
...
class TestFetchValueFunction(unittest.TestCase):
    ...
    def test_fetch_value(self):
        ...
        self.assertRaises(KeyError, fetch_value, ap, project,'doesnt_exist')
