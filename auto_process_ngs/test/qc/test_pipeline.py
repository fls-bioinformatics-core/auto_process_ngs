...
class TestQCPipeline(BaseQCPipelineTestCase):
    ...
    def test_qcpipeline_wildcard_read_in_protocol(self):
        """
        QCPipeline: handle wildcard read in QC protocol specification
        """
        # Make mock QC executables
        MockFastQC.create(os.path.join(self.bin,"fastqc"))
        os.environ['PATH'] = "%s:%s" % (self.bin,
        ...
        p.create(top_dir=self.wd)
        # Custom minimal QC protocol
        protocol = QCProtocol(name="wildcard_read",
                              description="Wildcard read test",
                              seq_data_reads=['r*'],
                              index_reads=None,
                              qc_modules=("fastqc",))
        # Set up and run the QC
        ...
        qc_info = AnalysisProjectQCDirInfo(
            os.path.join(self.wd, "PJB", "qc", "qc.info"))
        self.assertEqual(qc_info.protocol,"wildcard_read")
        self.assertEqual(qc_info.protocol_specification,
                         "wildcard_read:'Wildcard read test':"
                         "seq_reads=[r1,r2]:"
                         "index_reads=[]:"
                         "qc_modules=[fastqc]")
        self.assertEqual(qc_info.organism, "Human")
