#######################################################################
# Unit tests for qc/pipeline.py (pipeline features)
#######################################################################
...
class TestQCPipeline(BaseQCPipelineTestCase):
    """
    Tests for pipeline features (biological samples etc)
    """
    def test_qcpipeline_with_biological_samples(self):
        """
        QCPipeline: biological samples defined for QC
        """
...
""")
    ...
    def test_qcpipeline_with_fastqs_split_by_lane(self):
        """
        QCPipeline: Fastqs split by lane for QC
        """
        # Make mock QC executables
        MockFastqScreen.create(os.path.join(self.bin,"fastq_screen"))
        MockFastQC.create(os.path.join(self.bin,"fastqc"))
