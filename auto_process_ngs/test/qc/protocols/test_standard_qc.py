#######################################################################
# Unit tests for qc/pipeline.py (standard protocols for SE/PE data)
#######################################################################
...
class TestQCPipeline(BaseQCPipelineTestCase):
    ...
    """
    def test_qcpipeline_standard_paired_end(self):
...
""")
    ...
    def test_qcpipeline_standard_single_end(self):
        ...
        self.assertEqual(qc_info.protocol_specification,
                         "standard:'Standard QC for single and paired-end data':"
                         "seq_reads=[r1]:index_reads=[]:qc_modules=[fastq_screen,fastqc,picard_insert_size_metrics,"
                         "qualimap_rnaseq,rseqc_genebody_coverage,rseqc_infer_experiment,sequence_lengths]")
        self.assertEqual(qc_info.organism,"Human")
        self.assertEqual(qc_info.seq_data_samples,"PJB1,PJB2")
        self.assertEqual(qc_info.fastq_dir,
    ...
    def test_qcpipeline_standard_pe(self):
        """QCPipeline: standardPE QC run (paired-end data)
        """
        # Make mock QC executables
        MockFastqScreen.create(os.path.join(self.bin,"fastq_screen"))
        MockFastQC.create(os.path.join(self.bin,"fastqc"))
...
""")
    ...
    def test_qcpipeline_standard_se(self):
        """QCPipeline: standardSE QC run (single-end data)
        """
        # Make mock QC executables
        MockFastqScreen.create(os.path.join(self.bin,"fastq_screen"))
        MockFastQC.create(os.path.join(self.bin,"fastqc"))
