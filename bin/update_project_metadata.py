...
if __name__ == "__main__":
    ...
    for p in projects:
        ...
        if not has_metadata_file:
            ...
            missing_data = []
            for attr in ('run','user','PI','samples',
                         'library_type','organism','platform'):
                if p.info[attr] is None:
                    missing_data.append(attr)
            if missing_data:
                print("%s: missing values for %s" %
                      (p.name,', '.join(["'%s'" %
                                         x for x in missing_data])))
            else:
                print("%s: ok" % p.name)
        # Do updates
        if args.update:
            if not has_metadata_file:
                sys.stderr.write("No metadata file: use -i/--init to "
                                 "create one first\n")
                sys.exit(1)
